...
from lib.data_utils.img_utils import (
    get_single_image_crop, 
    convert_cvimg_to_tensor, 
    get_image_masked, get_bbox_from_kp2d, scale_bbox, get_image_kp2d_crop)


def extract_features(model, video, bbox, debug=False, batch_size=200, kp_2d=None, dataset=None, scale=1.3, mode=0, ratio=None):
    '''
    ...
    if dataset == 'insta':
        if mode==1: # masking
            bbox = get_bbox_from_kp2d(kp_2d)
            if(bbox.shape[0]==4):
                bbox = np.transpose(bbox,[1,0])
            video = torch.cat(
                # first convert to tensor then mask, not to affect the normalization
                [get_image_masked(convert_cvimg_to_tensor(image), bbox=bbox[id][:], ratio=ratio).unsqueeze(0) for id, image in enumerate(video)], dim=0
            ).to(device)
        elif mode==2: # cropping
            no_invisible = lambda x:(np.where(x[:,2]==0)[0].size < 3)
            bbox = get_bbox_from_kp2d(kp_2d)
            if(bbox.shape[0]==4):
                bbox = np.transpose(bbox,[1,0])
            db = {
            ...
            for id, image in enumerate(video):
                if no_invisible(kp_2d[id][:]):
                    img, kp2d = get_image_kp2d_crop(image, bbox=bbox[id][:], kp_2d=kp_2d[id][:].copy(), ratio=ratio)
                else:
                    img = convert_cvimg_to_tensor(image).unsqueeze(0)
                    kp2d = np.expand_dims(kp_2d[id][:], axis=0)
                db['video'].append(img)
                db['kp_2d'].append(kp2d)
            video = torch.cat(db['video']).to(device)
            kp_2d_cropped = np.concatenate(db['kp_2d'], axis=0)
        else:
            video = torch.cat(
        ...
        # crop bbox locations
        # default cropped size: 224x224
        if mode==1: # masking
            video = torch.cat(
                [get_single_image_crop(get_image_masked(image, bbox), bbox, scale=scale).unsqueeze(0) for image, bbox in zip(video, bbox)], dim=0
            ).to(device)
        else:
            video = torch.cat(
    ...
    if batch_size==-1: frames = (video,)
    else: frames = torch.split(video, batch_size)
    ...
    if mode==2 and dataset=='insta':
        return features.numpy(), kp_2d_cropped
    else:
