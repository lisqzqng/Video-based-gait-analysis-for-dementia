...
def rotate_2d(pt_2d, rot_rad):
    """rotate 2d point(s) according to the rotation"""
    x = pt_2d[0]
    y = pt_2d[1]
...
def gen_trans_from_patch_cv(c_x, c_y, src_width, src_height, dst_width, dst_height, scale, rot, inv=False):
    ...
    # get the Affine Transformation that maps the 2 triangles #
    if inv:
...
def generate_patch_image_cv(cvimg, c_x, c_y, bb_width, bb_height, patch_width, patch_height, do_flip, scale, rot):
    ...
    if bb_width!=bb_height:
        ...
        s = patch_height/max(bb_height, bb_width)
        ptrans = gen_trans_from_patch_cv(c_x, c_y, bb_width, bb_height, int(s*bb_width), int(s*bb_height), scale, rot, inv=False)
        img = cv2.warpAffine(img, ptrans, (int(s*bb_width), int(s*bb_height)),
                            flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)
        dx = patch_width/2 - img.shape[1]/2
        dy = patch_width/2 - img.shape[0]/2
        trans = np.array([[1,0,dx],[0,1,dy]]).astype(np.float64)
    else:
...
def transfrom_keypoints(kp_2d, center_x, center_y, width, height, patch_width, patch_height, do_augment):
    ...
    # convert bbox squared off if bbox rectangular
    if width!=height:
...
def get_bbox_limit(scale, neg=True):
    """
    convert [c_x, x_y, w, h] into [x_min, x_max, y_min, y_max]
    \nArgs: if `neg`, return [-x_min, x_max, -y_min, y_max] instead
    """
    b1, b2 = np.array([1,0,1/2*scale]), np.array([1,0,-1/2*scale])
    M = np.array([
                np.hstack([b2,0]),
                np.hstack([b1,0]),
                np.hstack([0,b2]),
...
def get_single_image_crop(image, bbox, scale=1.3):
    ...
    if bbox[2]!=bbox[3]:
        h, w = img.shape[:2]
        limit = np.matmul(get_bbox_limit(scale=scale),bbox)
        if (limit>np.array([0,w,0,h])).any():
            scale=1.
    crop_image, _ = generate_patch_image_cv(
        cvimg=img,
        c_x=bbox[0],
    ...
    if visualize:
        ...
        im = crop_image
        if(im.shape[0]==3):im = np.transpose(im, [1,2,0])
        plt.imshow(im)
        plt.show()
    crop_image = convert_cvimg_to_tensor(crop_image)

    bbox[2:] *= scale # modify the bbox with actual scale
...
def get_bbox_from_kp2d(kp_2d):
    """get squared-off bbox from kp2d"""
...
def get_image_masked(image, bbox, ratio=[0.6, 0.3]):
    """ bbox [c_y, c_x, w, h] """
    if len(ratio)==1: ratio = [ratio[0], 1-ratio[0]]
    [xrate, yrate] = ratio
    #w, h = bbox[2]-bbox[0]+1, bbox[3]-bbox[1]+1
    w, h = bbox[2], bbox[3]
    bbox[0] = bbox[0] - w / 2
    bbox[1] = bbox[1] - h / 2
    # extend the rectangle throught the y axis by 9 pixels
    # Careful with the "x, y" <=> "width, height"
    y_start, x_start = max(round(bbox[0] + 0.5*w*(1 - xrate)),0), max(round(bbox[1])-8, 0)
    y_end, x_end = round(bbox[0] + 0.5*w*(1 + xrate)), round(bbox[1] + yrate*h)
    if isinstance(image, str):
        if os.path.isfile(image):
            image = cv2.cvtColor(cv2.imread(image), cv2.COLOR_BGR2RGB) # numpy (HxWxC)
            image[x_start:x_end+1,y_start:y_end+1,:] = 0
        else:
            print(image)
            raise BaseException(image, 'is not a valid file!')
    elif isinstance(image, np.ndarray) or isinstance(image, torch.Tensor): 
        if (np.where(np.array(image.shape)>100) == np.array([1,2])).all(): # (CxWxH)
            image[:,x_start:x_end+1,y_start:y_end+1] = 0
        elif ((np.where(np.array(image.shape)>100) == np.array([2,3])).all()): # (BxCxWxH)
            image[:,:,x_start:x_end+1,y_start:y_end+1] = 0
        else:
            raise ValueError(f"Invalid input shape : {image.shape}")
    else:
        raise TypeError(f"Unexpected image type: {type(image)}.")
    #cv2.rectangle(image, start_pt, end_pt, color, thickness=-1)
    visualize = False
    if visualize:
        import matplotlib.pyplot as plt
        im = image if isinstance(image, np.ndarray) else image.numpy()
        if(im.shape[0]==3):im = np.transpose(im, [1,2,0])
        plt.imshow(im)
        plt.close()
...
def scale_bbox(bbox, ratio=[0.8]):
    [c_x, c_y, w, h] = bbox.tolist()
    assert w == h
    ort = [random.randint(0,2), random.randint(0,2)]
    x_increment = (1-ratio[0])*w/2 if ort[0]==0 else -(1-ratio[0])*w/2
    y_increment = (1-ratio[0])*h/2 if ort[1]==0 else -(1-ratio[0])*h/2
    c_x += x_increment
    c_y += y_increment
    w = h = w*ratio[0]
    w=h=max(w,h)
...
def crop_kp2d(bbox, kp_2d):
    """ set out-of-bound joints invisible """
    w, h = bbox[2], bbox[3]
    x_min = bbox[0] - w / 2
    y_min = bbox[1] - h / 2
    x_max = bbox[0] + w / 2
    y_max = bbox[1] + h / 2
    for np_jt in range(kp_2d.shape[0]):
        if (kp_2d[np_jt,:2]==112).all(): continue
        if np.logical_or.reduce((kp_2d[np_jt,0]<x_min, kp_2d[np_jt,0]>x_max, kp_2d[np_jt,1]<y_min, kp_2d[np_jt,1]>y_max)):
            kp_2d[np_jt,:-1] = 112 # set to 112 to distinguish from the 'not-available' jts
            kp_2d[np_jt,-1] = 0
    return kp_2d

def get_image_kp2d_crop(image, bbox, kp_2d, ratio=0.8, scale=1.2, crop_size=224):
    # no_invisible = lambda kp_2d:(np.where(kp_2d[:,2]==0)[0].size < 3)
    if isinstance(image, str):
        if os.path.isfile(image):
            image = cv2.cvtColor(cv2.imread(image), cv2.COLOR_BGR2RGB)
        else:
        ...
        raise('Unknown type for object', type(image))
    ## check if invisible joints already exists
    # if no_invisible(kp_2d):
    bbox = scale_bbox(bbox, ratio=ratio)
    kp_2d = crop_kp2d(bbox, kp_2d)
    crop_image, trans = generate_patch_image_cv(
        cvimg=image.copy(),
        c_x=bbox[0],
        c_y=bbox[1],
    ...
    crop_image = convert_cvimg_to_tensor(crop_image).unsqueeze(0)

    return crop_image, np.expand_dims(kp_2d, axis=0)