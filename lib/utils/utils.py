...
def load_ckpt_w_prefix(model, state_dict, prefix, strict=False, remove_lightning=False):
    """ prefix contains module name, no `model.`
    """
    pretrained_keys = state_dict.keys()
    new_state_dict = OrderedDict()
    for pk in pretrained_keys:
        if pk.startswith('model.'+prefix):
            new_state_dict[pk.replace('model.'+prefix, '')] = state_dict[pk]
        elif not pk.startswith('model'):
