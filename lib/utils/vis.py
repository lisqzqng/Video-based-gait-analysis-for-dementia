...
def batch_check_preds(images, preds, dataset='common'):
    """
    Check annotations of the input data.
    """
...
def show_preds(video, preds, max_video, dataset=''):
    with torch.no_grad():
        if isinstance(video, torch.Tensor):
            video = torch_vid2numpy(video) # NTCHW
    ...
    for batch_id in range(batch_size):
        ...
        for t_id in tqdm(range(tsize)):
            ...
            if visualize:
                copy_img = image.copy()
                for ind in range(single_pred['kp_2d'].shape[0]):
                    kp = (single_pred['kp_2d'][ind]+1)*112
                    copy_img = cv2.circle(copy_img, (int(kp[0]), int(kp[1])), thickness=1, radius=1, color=(0,255,0))
                cv2.imshow('preds_kp2d', copy_img)
                cv2.waitKey(1000)

            img = visualize_preds(image, single_pred, target_exists=False, dataset=dataset)
            result_video.append(img[np.newaxis, ...])
        
        if visualize: cv2.destroyWindow('preds_kp2d')
...
def draw_3d_skeleton(joints3d, ax_3d, dataset='spin'):
    """draw 3D skeleton from joint positions
    \n Attention with the color format (RGB, BGR)"""
    skeleton = eval(f'kp_utils.get_{dataset}_skeleton')()
    lcolor = (get_colors()['red']/255).tolist() # Blue
    pcolor = (get_colors()['green']/255).tolist()
    rcolor = (get_colors()['blue']/255).tolist() # Red
    ...
    for i,(j1,j2) in enumerate(skeleton):
        if dataset == 'common':
            ...
            color = lcolor if i % 2 == 0 else rcolor
        x, y, z = [joints3d[[j1,j2], c] for c in range(3)]
...
if __name__=='__main__':
    ...
    RADIUS = 1.7
    ax_3d.view_init(elev=-75, azim=-90)
    ax_3d.dist = 7.5

    for i in range(3):
        image = np.random.normal(size=(480, 640, 3))
        frame_j3d = np.random.normal(size=(49,3))
        ...
        draw_3d_skeleton(frame_j3d, ax_3d)
        w, h = fig.canvas.get_width_height()
        buf = np.fromstring(fig.canvas.tostring_argb(), dtype=np.uint8)
        buf.shape = (w, h, 4)
        rgb_img = buf[:,:,1:]
        plt.savefig(os.path.join('output/', f'{i:06d}.png'))
