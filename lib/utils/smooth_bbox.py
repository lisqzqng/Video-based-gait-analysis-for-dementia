...
def get_smooth_bbox_params(kps, vis_thresh=2, kernel_size=11, sigma=3, squared=True, scale=1.1):
    """
    ...
    Args:
        ...
        squared : to generate squared-off bounding box or not. If not, \
            scale the original (w, h) by the scaling factor

    Returns:
        Smooth bbox params [cx, cy, scale], start index, end index
    """
    bbox_params, start, end = get_all_bbox_params(kps, vis_thresh, squared=squared, scale=scale)
    smoothed = smooth_bbox_params(bbox_params, kernel_size, sigma)
    smoothed = np.vstack((np.zeros((start, 3)), smoothed)) if squared else np.vstack((np.zeros((start, 4)), smoothed))
    return smoothed, start, end


def kp_to_bbox_param(kp, vis_thresh, squared=True, scale=1.1):
    """
    ...
    if squared:
        ...
        wh = (max_pt-min_pt)*scale
        assert (wh>0).all()
        return np.append(center, wh)


def get_all_bbox_params(kps, vis_thresh=2, squared=True, scale=1.1):
    """
    ...
    start_index = -1
    bbox_params = np.empty(shape=(0, 3), dtype=np.float32) if squared \
        else np.empty(shape=(0, 4), dtype=np.float32)

    for i, kp in enumerate(kps):
        bbox_param = kp_to_bbox_param(kp, vis_thresh=vis_thresh, squared=squared, scale=scale)
