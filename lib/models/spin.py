...
from lib.utils.geometry import rotation_matrix_to_angle_axis, rot6d_to_rotmat, projection
...
class HMR(nn.Module):
    ...
    def forward(self, x, init_pose=None, init_shape=None, init_cam=None, n_iter=3, return_features=False):
        ...
        pred_output = self.smpl( # use smpl to get(calculate) 3D joint / vertice positions 
            betas=pred_shape,
...
class Regressor(nn.Module):
    def __init__(self, smpl_mean_params=SMPL_MEAN_PARAMS):
        ...
        init_shape = torch.from_numpy(mean_params['shape'][:].astype('float32')).unsqueeze(0)
        init_cam = torch.from_numpy(mean_params['cam']).unsqueeze(0)
        # Submodules as nn.Linear will be automatically registered, while variables as init_pose need to manually register
        self.register_buffer('init_pose', init_pose)
        self.register_buffer('init_shape', init_shape)
        self.register_buffer('init_cam', init_cam)
    ...
    def forward(self, x, init_pose=None, init_shape=None, init_cam=None, n_iter=3, J_regressor=None):
        ...
        pred_rotmat = rot6d_to_rotmat(pred_pose).view(batch_size, 24, 3, 3) # normalize pose
