...
class PoseHighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, multi_output=False):
        self.inplanes = 64
        ...
        # for iterative close-loop correction
    ...
    def forward(self, x):
        ...
        # s_feat = [x_list[-2], x_list[-3], x_list[-4]] # 14x14, 28x28, 56x56
        ...
        #     return s_feat, x
        # else:
        return x # always use the concatenated feature for the moment
    ...
    def init_weights(self, pretrained=''):
        ...
        if os.path.isfile(pretrained):
            ...
            try: pretrained_state_dict = pretrained_state_dict['state_dict']
            except Exception: pass
            else:
                self.pretrained_layers+=['final_layer', 'upsample_stage_2', 'upsample_stage_3', 'upsample_stage_4']
            ...
            logger.warning('hrnet: Please download pre-trained models if you are in TRAINING mode!')
...
class Hrnet():
    def __init__(self):
        self.model = get_pretrained_hrnet()

    def feature_extractor(self, images):
        seqlen, nc, h, w = images.shape
        features = self.model(images)
        assert features.shape==(seqlen, 480, 56, 56), logger.error(f"Wrong feature dimension: {features.shape[1:]}")
        return features
...
def get_hrnet_encoder(
    ...
    use_conv=False):
    """
    use hrnet_w32 as default.
    """
    cfg = get_cfg_defaults(pretrained_ckpt, width=32, downsample=downsample, use_conv=use_conv)
    model = PoseHighResolutionNet(cfg, multi_output=False)
