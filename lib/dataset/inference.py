...
class Inference(Dataset):
    def __init__(self,
        ...
        self.bboxes = bboxes
        self.bboxes[:,2:] *= scale  # will simultaneously change values in original `bboxes`
    ...
    def __getitem__(self, idx):
        ...
        if not self.use_gait_feat and self.has_keypoints:
            return norm_img, kp_2d
        elif self.use_gait_feat:
            return norm_img, bbox, np.ones(bbox.shape[:-1]+(1,))*np.array(img_size)*0.5,
        else:
            return norm_img
